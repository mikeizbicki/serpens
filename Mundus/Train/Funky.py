...
class ActorCriticPolicy(BasePolicy):
    ...
    def __init__(
        ...
        self.n_values = observation_space['events'].shape[-1]
    ...
    def _build(self, lr_schedule: Schedule) -> None:
        ...
        self.value_net = nn.Linear(self.mlp_extractor.latent_dim_vf, self.n_values)
...
class DictRolloutBuffer(RolloutBuffer):
    ...
    def reset(self) -> None:
        ...
        self.rewards = np.zeros((self.buffer_size, self.n_envs, self.n_values), dtype=np.float32)
        self.returns = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
        self.episode_starts = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
        self.values = np.zeros((self.buffer_size, self.n_envs, self.n_values), dtype=np.float32)
        self.log_probs = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
        self.advantages = np.zeros((self.buffer_size, self.n_envs, self.n_values), dtype=np.float32)
        self.generator_ready = False
    ...
    def compute_returns_and_advantage(self, last_values: th.Tensor, dones: np.ndarray) -> None:
        """
        FIXME:
        copied from super() to slightly change behavior
        """
        # Convert to numpy
        ...
        for step in reversed(range(self.buffer_size)):
            ...
            next_non_terminal = next_non_terminal[:, np.newaxis]
...
class VecMonitor(VecEnvWrapper):
    ...
    def __init__(
        ...
        #self.episode_returns = np.zeros([self.num_envs, self.n_values], dtype=np.float32)
        #self.episode_lengths = np.zeros(self.num_envs, dtype=np.int32)

    def reset(self) -> VecEnvObs:
        obs = self.venv.reset()
        self.n_values = obs['rewards'].shape[-1]
        logging.warning(f"self.n_values={self.n_values}")
        self.episode_returns = np.zeros([self.num_envs, self.n_values], dtype=np.float32)
        self.episode_lengths = np.zeros(self.num_envs, dtype=np.int32)
        return obs
