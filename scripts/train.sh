...
CUDA_VISIBLE_DEVICES=0 nohup python3 Mundus/train.py $common --features_dim=32 --pooling=max --screen_extractor=NatureCNN --screen_dim=512 --screen_downsample=1 --objects_extractor=None --revents_extractor=None > nohup/nohup.0 &
CUDA_VISIBLE_DEVICES=1 nohup python3 Mundus/train.py $common --features_dim=32 --pooling=max --center_player --objects_extractor=ObjectEmbeddingWithDiff --screen_extractor=SimpleCNN > nohup/nohup.1 &
